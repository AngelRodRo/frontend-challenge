...
const PlansPage: React.FC<Props> = () => {
  ...
  const [isForSomeoneElseSelected, setIsForSomeoneElseSelected] =
    useState(false);
    ...
    <div className={styles.PlanPage}>
      ...
      <div className={styles.PlanPage__content}>
        ...
        <div className={styles.PlanPage__optionsSection}>
          ...
          <p className={styles.PlanPage__subtitle}>
            Selecciona la opción que se ajuste más a tus necesidades.{" "}
          </p>

          <div className={styles.PlanPage__options}>
            <Card
              className={styles.PlanPage__option}
              selectable
              selected={isForMeCardSelected}
              onSelected={handleSelectionIsForMeCard}
            >
              <Icon name={IconName.forMePlan} />
              <h2 className={styles.PlanPage__optionTitle}>Para mi</h2>
              <p className={styles.PlanPage__optionDescription}>
            ...
            <Card
              className={styles.PlanPage__option}
              selectable
              selected={isForSomeoneElseSelected}
              onSelected={handleSelectionIsForSomeoneElseCard}
            >
            ...
            <div className={styles.PlanPage__plans}>
              {filteredPlans &&
                filteredPlans.map((plan, idx) => (
                  <PlanCard
                    key={idx}
                    recommended={recommendedPlansMap[plan.name]}
                    iconName={planNameIconMap[plan.name] ?? IconName.housePlan}
                    {...plan}
                    onSelected={handleSelectedPlan}
                  />
                ))}
            </div>
          )}
