  ...
  return (
    <Card className={styles.PlanCard}>
      <div style={{ position: "relative", flex: 1 }}>
        {recommended && (
          <Badge
            className={styles.PlanCard__recommended}
            text="Plan Recomendado"
          />
        )}
        <div className={styles.PlanCard__info}>
          <div className={styles.PlanCard__name}>
            <h2 className={styles.PlanCard__title}>{name}</h2>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <span className={styles.PlanCard__priceLabel}>
                Costo del Plan
              </span>
        ...
        <ul>
          {description.map((item, idx) => {
            return (
              <li className={styles.PlanCard__descriptionItem} key={idx}>
                {item}
              </li>
            );
          })}
